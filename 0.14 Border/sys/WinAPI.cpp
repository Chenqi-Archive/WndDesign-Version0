...
extern WndDesign::SysWndMng* master;
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    MouseMsgPara mouseMsg = { {GET_X_LPARAM(lParam),GET_Y_LPARAM(lParam)}, wParam };

    switch (msg) {
        case WM_MOUSEMOVE:
            master->ReceiveSysMsg(hWnd, Msg::MOUSE_MOVEON, &mouseMsg);
            break;
        case WM_LBUTTONDOWN:
            SetCapture(hWnd);
            master->ReceiveSysMsg(hWnd, Msg::MOUSE_LEFTDOWN, &mouseMsg);
            break;
        case WM_LBUTTONUP:
            master->ReceiveSysMsg(hWnd, Msg::MOUSE_LEFTUP, &mouseMsg);
            ReleaseCapture();
            break;
        case WM_RBUTTONUP:
            master->ReceiveSysMsg(hWnd, Msg::MOUSE_RIGHTUP, &mouseMsg);
            break;
        case WM_PAINT:
            const Bitmap* bitmap;
            master->ReceiveSysMsg(hWnd, Msg::WND_PAINT, &bitmap);
            SysPaintWnd(hWnd, bitmap);
            break;
        case WM_DESTROY:
            if (master->SubWndCnt() == 0) { PostQuitMessage(0); return 0; }
...
SysID SysCreateWnd(Rect rect) {
    if (!WndClassRegistered) {
        if (RegisterBlankClass()) { WndClassRegistered = true; } else { return NULL; }
    }

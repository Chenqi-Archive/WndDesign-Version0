...
bool Wnd::ChangeBG(const Figure& figure){
	delete background;
	WndID self = myself;
	background = figure.Clone(&self);
	RefreshBG();
	return true;
}
...
Wnd::Wnd(Size size, WndID parent, WndID myself, const Figure& figure):parent(parent),myself(myself), display(size){
	// May add some additional features.
	WndID self = myself;
	background = figure.Clone(&self);  // Self's wndID is for animations.
	background->PaintOn(display);
}

...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        case WM_MOUSEMOVE:
            master->ReceiveSysMsg(hWnd, Msg::MOUSE_MOVEON, &mousePos);
            break;
        case WM_LBUTTONDOWN:
            SetCapture(hWnd);
            master->ReceiveSysMsg(hWnd, Msg::MOUSE_LEFTDOWN, &mousePos);
            break;
        case WM_LBUTTONUP:
            master->ReceiveSysMsg(hWnd, Msg::MOUSE_LEFTUP, &mousePos);
            ReleaseCapture();
            break;
        case WM_RBUTTONUP:
            master->ReceiveSysMsg(hWnd, Msg::MOUSE_RIGHTUP, &mousePos);
            break;
        case WM_PAINT:
            const Bitmap* bitmap;

...
public:
	...
	inline bool Set(Pos pos, const T& item) {
		if ((uint)pos.x < size.width && (uint)pos.y < size.height) { bits[pos.y * size.width + pos.x] = item; return true; }
		return false;
	}
	inline bool Get(Pos pos, T& item) const {
		if ((uint)pos.x < size.width && (uint)pos.y < size.height) { item = bits[pos.y * size.width + pos.x]; return true; }
		return false;
	}
	inline void DirectSet(Pos pos, const T& item) {  // No boundary checking!
		bits[pos.y * size.width + pos.x] = item;
	}
	inline void DirectGet(Pos pos, T& item) const {  // No boundary checking!
		item = bits[pos.y * size.width + pos.x];
	}

...
bool Wnd::RefreshBG()
{
	background->PaintOn(display);  // Copied twice, can be optimized.
...
Wnd::Wnd(Size size, WndID parent, WndID myself, const Shape& shape):parent(parent),myself(myself), display(size){
	// May add some additional features.
	WndID self = myself;
	background = shape.Clone(&self);  // Self's wndID is for animations.
	background->PaintOn(display);
}
...
bool Wnd::MsgHandler(Msg msg, void* para) {
	...
	case MSG_WND_CHANGEBG: if (para != nullptr) { ChangeBG(*(Shape*)para); }else { RefreshBG(); }; break;
